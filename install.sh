...
###############################################################################
# Install!          http://patorjk.com/software/taag/ ( font: Script )
###############################################################################
...
if [ -x rvm ]; then
	...
	print_success_muted "RVM already installed. Skipping."
fi
...
###############################################################################
chapter "Adding customisations"
if [ -f "$HOME/.customisations" ]; then
	if ask "Do you want to add customisations?" Y; then
		. "$HOME/.customisations"; printf "\n  Installed customisations \n";
	else
		print_success_muted "Customisations declined. Skipped.";
	fi
else
	print_warning "No ~/.customisations found. Skipping."
fi
